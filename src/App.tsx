...
import burnChrome from "./assets/burn-chrome.png";
import searchIcon from "./assets/input-icon.png";
import notWhiteX from "./assets/logo_circle_white.svg";
...
function BurnersModal({
  burnersList,
  onClose,
}: {
  burnersList: BurnerData[];
  onClose: () => void;
}) {
        ...
        <div className="burners-list">
          ...
          <ul>
            {burnersList.map((burner, index) => (
              <li key={index}>
                {burner.burnerAddress} - {burner.burnerAmount.toLocaleString()}{" "}
                $NOT
              </li>
            ))}
          </ul>
...
function App() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://api.mainnet.hiro.so/extended/v1/address/" +
            userPrincipal +
            "/balances"
        );
  ...
  useEffect(() => {
    const fetchData2 = async () => {
      try {
        const response = await fetch(
          "https://api.mainnet.hiro.so/v2/contracts/call-read/SPEMB0KQRD7PWKY2W2J2Y1Y6Q9YBJ702DWQADE7V/not-incinerator-v3/get-burned-amount",
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    ...
    const fetchData3 = async () => {
      try {
        const response = await fetch(
          "https://api.mainnet.hiro.so/v2/contracts/call-read/SPEMB0KQRD7PWKY2W2J2Y1Y6Q9YBJ702DWQADE7V/not-incinerator-v3/get-tot-burns",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const fetchData4 = async (index: number): Promise<BurnerData> => {
      try {
        ...
        const response = await fetch(
          "https://api.mainnet.hiro.so/v2/contracts/call-read/SPEMB0KQRD7PWKY2W2J2Y1Y6Q9YBJ702DWQADE7V/not-incinerator-v3/get-burns-list",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className="home-container">
      ...
      <div className="burn-panel-wrapper">
        ...
        <div className="burn-panel-rect">
          <div className="burn-panel-container">
            ...
            >
              <div
                className={`input-rect1 ${
                  isInputHovered || isInputActive ? "active" : ""
                }`}
              />
            ...
            <div className="burn-button" onClick={handleBurnClick}>
              BURN
            </div>
            <div className="burn-descr-text">
              You will burn {amount.toLocaleString()} $NOT
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="burners-text" onClick={handleBurnersTextClick}>
        Show $NOT Burners list
      </div>
      {showBurnersModal && (
        <BurnersModal burnersList={burnersList} onClose={handleCloseModal} />
      )}
    </div>
  );
}
